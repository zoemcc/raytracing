...
fn time_to_vertex_buffer(time: f32, angular_speed: f32, aspect_ratio: f32, device_clone: Arc<Device>) -> Arc<CpuAccessibleBuffer<[Vertex]>> {
    let radians = time * angular_speed;
    CpuAccessibleBuffer::from_iter(
        ...
        [
            Vertex {
                position: [-0.5, -0.25 * aspect_ratio],
            },
            Vertex {
                position: [0.0, 0.5 * aspect_ratio],
            },
            Vertex {
                position: [0.25, -0.1 * aspect_ratio],
            },
        ]
            .iter()
            .map(|v| apply_rotation(radians, v))
    )
        .unwrap()
}

fn apply_rotation(radians: f32, vertex: &Vertex) -> Vertex {
    let (rcos, rsin) = (f32::cos(radians), f32::sin(radians));
    let (vx, vy) = (vertex.position[0], vertex.position[1]);
    Vertex {
        position: [rcos * vx + rsin * vy, -rsin * vx + rcos * vy]
    }
